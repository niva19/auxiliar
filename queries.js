...
function editProject(req, res, next) {
  console.log("proyectos editar", req.body);
  db.tx(t => {
    return t.batch([
      t.none('UPDATE proyecto SET nombreProyecto = ${nombreProyecto}, direccion = ${direccion}, tipoproyecto = ${tipoProyecto}, tipoobra = ${tipoObra}, descripcion = ${descripcion}, fechainicio = ${fechaInicio}, fechafinaliza = ${fechaFinaliza}, estado = ${estado}, banco = ${banco}  where nombreproyecto = ${nombreProyecto}', req.body),
    ]);
  })
    .then(data => {
      res.status(200)
